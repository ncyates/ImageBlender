...
if __name__ == "__main__":
    ...
    for i in range(len(filesChunks)):
        compareProcesses.append(multiprocessing.Process(target=compare,args=('p' + str(i), acceptQ,filesChunks[i], acceptDir, rejectDir,stdDim)))
    ...
    sys.stdout.write('\n\tFinished SSIM in %s seconds' % (time.time() - start_time))

    while acceptQ.qsize() != 0:
        filteredFileNames.append(sourceDir + '/' + acceptQ.get()[0])
